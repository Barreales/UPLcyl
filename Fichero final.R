...
datos$P5_1 <- factor(datos$P5_1, levels = c(1, 2, 3, 4, 5, 6, 7, 95, 97, 98, 99),
                     labels = c("Prensa, en formato impreso", "Prensa, en formato digital", "Televisión", "Radio", "Redes sociales", 
                                "Contactos personales, reuniones, mítines, etc.", " Otros medios", "Ningún medio más", 
                                "No se informó, no le interesa la política", "N.S.", "N.C."))
...
datos$FIDELID <- factor(datos$FIDELID, levels = c(1, 2, 3, 4, 5, 6, 8, 9),
                        labels = c("Votan siempre por el mismo partido", "Suelen votar por el mismo partido",
                                   "Varian su partido", "Votan en blanco o nulo",
                                   "No suelen votar", "Es la primera vez que votan", "No recuerda/N.S.", "N.C."))
...
## Regresiones
datos$P5_1 <- relevel(datos$P5_1, ref = "No se informó, no le interesa la política") #Calcular el logit sobre "No se informó"
datos$CLASESUB <- relevel(datos$CLASESUB, ref = "Clase media-media") #Calcular el logit sobre clase media-media
datos$FIDELID <- relevel(datos$FIDELID, ref = "Suelen votar por el mismo partido") #Calcular el logit sobre "voto suele ser el mismo". Que se ha considerado el valor intermedio
...
datos$FIDELID <- factor(datos$FIDELID, levels = c("Votan siempre por el mismo partido", "Suelen votar por el mismo partido",
                                                  "Varian su partido")) #Eliminar categorias sin valores significativos
